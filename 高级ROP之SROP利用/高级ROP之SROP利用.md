...
这样通过对 rsp 的设置可以保证 srop 链依次进行调用
...
一般以 sys\_read、sys\_write 等伪 c 代码形式出现
...
然后 rsp 位置设置成了一个 bss 段的地址，实际上是产生了一个类似于栈迁移的效果，使 rsp 到对应位置继续向下执行代码
...
三段式的构造，open，read，write，将 flag 字符串和垃圾数据放在一起构造栈溢出
