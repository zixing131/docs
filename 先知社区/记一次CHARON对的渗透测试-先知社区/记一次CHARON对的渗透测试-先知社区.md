...
猜测可能存在 waf 拦截，然后经过测试，将 UNION SELECT 都使用大写进行测试，可以成功测试注入。
...
然后输入 cmd.php，重新上传.php.jpg 格式的文件。成功上传。
...
利用 supershell 成功读到了 root.txt，成功获得了第二个 flag。
