...
对于这个 handler 就很类似于 Spring 中的 controller，直接去处理我们的请求
...
1.如果在我们的 POST 参数中有`parameters`,获取到后变成 JSON 数组，然后将数组里的值给 Array 返回回去。
...
因为我们传入的 format 是 3，所以直接跟`Binary`匹配到，利用的是`BinaryFormatterPorxy`
