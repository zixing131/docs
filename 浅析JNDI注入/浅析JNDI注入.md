...
此时假设使用 rmi 协议 客户端通过 lookup 函数请求上面 bind 设置的 Exploit
...
可以看到在 register 端有一个 decodeObject 方法 跟进
...
然后和下面 ldap 一样的 loadclass 然后实例化
...
我们在构造 reference 的时候 可以传参 factory 这个过程写在 server 中
